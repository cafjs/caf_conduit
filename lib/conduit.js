...
var newToken = function(type, name, args, label, mArray) {
    return { 'type': type, 'name' : name, 'args' : args, 'label' : label,
             'children' : mArray};
};
...
var newConduit = function(methodNames, initStack) {
    ...
    var seqPar = function(n, f) {
        ...
        if (that.__length__() >= nFrames) {
            ...
            var m = f(mArray);
            return newConduit(methodNames, that.__push__(m));
        } else {
            return null;
        }
    };
